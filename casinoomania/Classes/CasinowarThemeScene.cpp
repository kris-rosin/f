...
Scene * CasinowarThemeScene::createScene()
{
    auto scene = Scene::create();
    
    auto layer = CasinowarThemeScene::create();
    scene->addChild(layer);

    return scene;
}

CasinowarThemeScene::CasinowarThemeScene()
{
...
bool CasinowarThemeScene::init()
{
    ...
    //topbar
    auto topbar = TopBar::create(std::bind(&CasinowarThemeScene::onBackButtonPressed, this));
    ...
    for (int i = 0; i < CasinowarConfig::Themes.size(); i++)
    {
        ...
        theme->addTouchEventListener(std::bind(&CasinowarThemeScene::onButtonPressed, this, std::placeholders::_1, std::placeholders::_2));
...
void CasinowarThemeScene::onEnter()
{
    Layer::onEnter();
...
void CasinowarThemeScene::onButtonPressed(Ref * sender, ui::Widget::TouchEventType event)
{
...
void CasinowarThemeScene::ProcessButtonAction(cocos2d::ui::Button * button)
{
    for (int i = 0; i < CasinowarConfig::Themes.size(); i++)
    {
        if (CasinowarConfig::Themes[i] == button->getName())
        {
            ...
            auto scene = LoadingSplash::createScene(LoadingSplash::CASINOWAR_GAME, button->getName());
...
void CasinowarThemeScene::onBackButtonPressed()
{
